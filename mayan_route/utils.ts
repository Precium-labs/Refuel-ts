...
export function txStatusToReceipt(txStatus: TransactionStatus): routes.Receipt {
    ...
    const destinationTxs = txStatus.txs
        ...
        type RefundTx = {
            chain: "Solana" | "Ethereum" | "Arbitrum" | "Optimism" | "Base"| "Terra" | "Bsc" | "Polygon" | "Avalanche" | "Oasis" | "Algorand" | "Aurora" | "Fantom"| "Moonbeam"  | "Karura" | "Acala" | "Klaytn" | "Celo" | "Near" | "Moonbeam" | "PolygonSepolia";
            txid: string;
        };
    

        const refundTxs: RefundTx[] = [];

        if (txStatus.refundTxHash) {
            refundTxs.push({
                chain: fromMayanChainName(txStatus.refundChain as MayanChainName) as RefundTx['chain'],
